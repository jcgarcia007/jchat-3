  ...
  return (
    <div style={{ flexShrink: 0, width: 150 }}>
      {item.photo_url ? (
        ...
        <div style={{ width: 150, height: 110, borderRadius: 10, background: "var(--bg-surface)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>🍽️</div>
      )}
      <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)", marginTop: 8, lineHeight: 1.25 }}>{item.name}</div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginTop: 4 }}>
        <span style={{ fontSize: 12.5, fontWeight: 700, color: "var(--color-gold)" }}>
...
export default function FullscreenType({
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "6px 0 40px" }}>
      {/* Discreet header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "10px 24px 18px" }}>
        <span style={{ fontSize: 12, fontWeight: 900, letterSpacing: "4px", textTransform: "uppercase", color: "var(--color-gold)" }}>
          {business.name}
        </span>
        <button
          type="button"
      ...
      {nonEmpty.map((cat, i) => {
        ...
        return (
          <div key={cat.id} style={{ borderTop: "0.5px solid var(--border-subtle)" }}>
            <button
              type="button"
            ...
            >
              <span style={{ fontSize: 11, color: "var(--color-gold)", flexShrink: 0 }}>{NUM(i)}</span>
              <span style={{ fontSize: 34, fontWeight: 500, color: "var(--text-primary)", lineHeight: 1 }}>{cat.name}</span>
              <span style={{ marginLeft: "auto", fontSize: 10, letterSpacing: "2px", color: "var(--text-tertiary)", flexShrink: 0, alignSelf: "center" }}>
