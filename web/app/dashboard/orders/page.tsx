 ...
 * Filters: All / Pending / In progress / Completed. Each row expands to show its
 * items, total and timing. Uses --db-* tokens only.
 */
...
const STATUS_META: Record<string, { label: string; bg: string; color: string }> = {
  ...
  delivered: { label: "Completed", bg: "rgba(34,197,94,0.14)", color: "var(--db-success)" },
...
const DEMO_ORDERS: Order[] = [
  {
    id: "demo-1001", status: "confirmed", order_type: "table", total_cents: 2800, eta_minutes: 15, created_at: new Date(Date.now() - 120000).toISOString(),
    items: [
  ...
  {
    id: "demo-1000", status: "preparing", order_type: "counter", total_cents: 1550, eta_minutes: 8, created_at: new Date(Date.now() - 900000).toISOString(),
    items: [{ id: "i3", qty: 1, price_cents: 1550, special_instructions: null, name: "Caesar Salad" }],
  },
  {
    id: "demo-0999", status: "delivered", order_type: "table", total_cents: 1900, eta_minutes: null, created_at: new Date(Date.now() - 7200000).toISOString(),
    ...
    void (async () => {
      ...
      try {
        ...
        const { data: ordersData, error: ordErr } = await supabase
          .from("orders")
          .select("id, status, order_type, total_cents, created_at, eta_minutes")
          .eq("business_id", res.business.id)
          .order("created_at", { ascending: false })
          .limit(200);
        if (ordErr) throw ordErr;

        const base = (ordersData ?? []) as Omit<Order, "items">[];
  ...
  return (
    <div style={{ maxWidth: "960px" }}>
      ...
      <p style={{ fontSize: "14px", color: "var(--db-text-secondary)", marginBottom: "20px" }}>
        Incoming, in-progress, and completed orders for your business.
      </p>
      ...
      {loading ? (
        ...
        <div style={{ padding: "40px", textAlign: "center", color: "var(--db-text-secondary)", fontSize: "14px", background: "var(--db-bg-surface)", border: "1px solid var(--db-border)", borderRadius: "12px" }}>
          No {filter === "all" ? "" : activeFilter.label.toLowerCase() + " "}orders.
        </div>
      ) : (
        <div style={{ background: "var(--db-bg-surface)", border: "1px solid var(--db-border)", borderRadius: "12px", overflow: "hidden" }}>
          {/* header */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 130px 90px 110px 110px", gap: "12px", padding: "12px 20px", fontSize: "11px", fontWeight: 700, color: "var(--db-text-tertiary)", letterSpacing: "0.05em", textTransform: "uppercase", borderBottom: "1px solid var(--db-border)" }}>
            <span>Order</span>
            <span>Status</span>
            ...
            return (
              <div key={o.id} style={{ borderBottom: "1px solid var(--db-border)" }}>
                <div
                  ...
                  style={{ display: "grid", gridTemplateColumns: "1fr 130px 90px 110px 110px", gap: "12px", padding: "14px 20px", alignItems: "center", cursor: "pointer" }}
                >
                  <span style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px", fontWeight: 600, color: "var(--db-text-primary)" }}>
                    {open ? <IconChevronUp size={15} /> : <IconChevronDown size={15} />}
                    #{o.id.slice(0, 8)}
                    {o.order_type && (
                      <span style={{ fontSize: "11px", fontWeight: 500, color: "var(--db-text-tertiary)", textTransform: "capitalize" }}>· {o.order_type}</span>
                    )}
                  </span>
                  <span>
                    <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: "999px", fontSize: "12px", fontWeight: 600, background: meta.bg, color: meta.color }}>
                      {meta.label}
                    </span>
                  </span>
                ...
                {open && (
                  <div style={{ padding: "0 20px 16px 43px", background: "var(--db-bg-base)" }}>
                    {o.items.length === 0 ? (
